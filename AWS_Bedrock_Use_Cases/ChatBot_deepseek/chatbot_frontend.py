...
import  ChatBotBackend as demo  #**Import your Chatbot file as demo

#2 Set Title for Chatbot - https://docs.streamlit.io/library/api-reference/text/st.title
st.title("Hi, This is Chatbot Almighty :sunglasses:") # **Modify this based on the title you want in want
...
input_text = st.chat_input("Chat with Kalra's Bedrock Bot here") # **display a chat input box
if input_text: 
    ...
    chat_response = demo.demo_conversation(input=input_text, memory=st.session_state.memory) #** replace with ConversationChain Method name - call the model through the supporting library
